...
- **选号策略**：默认使用趋势参考，并提供均衡生成、随机生成、数据参考和分层理论模型。
...
- **趋势参考**：默认模式，综合全部历史数据与最近 100 期走势，参考冷热、遗漏、奇偶、区间分布和历史频率生成娱乐参考号码。
- **均衡生成**：在合法随机号码里筛选结构更均衡的组合。
- **随机生成**：只按彩种规则生成合法号码。
- **数据参考**：参考历史号码频率，但不把历史数据视为未来结果依据。
- **分层理论模型**：结合官方奖级结构、历史数据和低权重随机扰动生成参考组合。
